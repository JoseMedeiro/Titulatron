...
%
%%
function grafequi = Grafico_Concentracao(app, UIAxes, Values)
    
    % RHn Plot
    for c = 1:size(Values.CONCENTRACAO.DADOS,1)
        PlotP(c)            = plot(UIAxes, Values.ABSISSA(1,:)*10^(3), Values.CONCENTRACAO.DADOS(c,:));
        if c == 1
    ...
    ylabel(UIAxes, 'Concentração (mol/L)');
    % Legenda
    Celula_Texto =  cellstr(Values.CONCENTRACAO.LEGENDAS); 
