...
%
%%
function grafequi = Grafico_Incrementos(app, UIAxes, Values)
    
    % R Plot
    for c = 1:size(Values.ERROS.DADOS,1)
        PlotP(c)            = plot(UIAxes, Values.ABSISSA(1,:)*10^(3), Values.INCREMENTOS.DADOS(c,:));
        if c == 1
    ...
    ylabel(UIAxes, 'Incrementos das Reações (mol/L)');
    % Legenda
    Celula_Texto =  cellstr(Values.INCREMENTOS.LEGENDAS); 
